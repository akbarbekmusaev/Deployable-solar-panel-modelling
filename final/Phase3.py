...
def phase3(p, S2):
    ...
    #setting variables from s2
    thet,phi,thetdot,phidot = S2.y
    t2 = S2.t
    
    #referencing variables from p
    g       = p[0]
    m_B     = p[1]
    M_CW    = p[2]
    M_P     = p[3]
    ...
    #setting z2 and v2 arrays based on derived formula 
    ...
    r_p = ((3*M_P)/(4*rho_wat*np.pi))**(1/3) # radius of projectile
    A_p = (np.pi)*(r_p)**2 # cross sectional surface area of projectile
    ...
    def f3(t, z):
        ...
        dxdt = vx #velocity x
        dydt = vy #veloicty y
        dvxdt = -M_P/(F_drag_x) #acceleration x
        dvydt = (-F_drag_y - M_P*g) / M_P #acceleration y
        
        return dxdt, dydt, dvxdt, dvydt

    # Step 6: Define the ODE solver options
    ...
    initial_conditions = np.concatenate([z2, v2]) #preparing array for solve_ivp
   
    # Step 8: Run solve_ivp to find time (t3) and states (z3)
    S3 = solve_ivp(
        f3, [t2[-1], 20], initial_conditions,
