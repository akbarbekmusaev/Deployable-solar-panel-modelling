...
def create_animation_spyder(sol, theta_finishing):
    ...
    return ani
def create_animation_damper(sol, theta_finishing):
    t_animation = sol.t
    x_animation = sol.y[0, :] + np.pi / 2
    ani = animate_sprung_pendulum(t_animation, x_animation)
    ani.save('animation.gif', writer=PillowWriter(fps=30))
    webbrowser.open('animation.gif')
...
def create_animation_spyder_damper(sol, theta_finishing):
    ...
    return ani
sol = openingmodel(theta_initial, theta_finishing, speed_initial, T_stall, omega_max, gearRatio, c)
create_animation_damper(sol, theta_finishing)